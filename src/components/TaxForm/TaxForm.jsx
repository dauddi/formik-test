					...
					return (
						<Form className="w-full">
							...
							<div className="flex border-2 border-gray-250 m-2 w-auto md:w-4/12 mb-4 rounded" >
								...
								<Field 
									className="w-full border-0 focus:outline-none pr-10 pl-2 py-1" 
							...
							{filteredData.map(group => (
								<CategoryCheckbox key={group.category} category={group.category} checkedState={values.Applied_to} className="container mt-4 gap-2 w-auto">
									{(group.items).map(item => (
										<ItemCheckbox key={item.id} Applicable_items={values.Applicable_items} checkedCategories={values.checkedCategories} itemObject={item} checkedState={values.Applied_to} />
									))}
								</CategoryCheckbox>
							))}
